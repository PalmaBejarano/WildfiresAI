...
# Export a CSV summary table
summary_file_path = 'summary_table.csv'

with open(summary_file_path, 'w') as csvfile:
    csvfile.write("material_id,formula_pretty,band_gap,energy_above_hull,density\n")
    for material in mp_results:
        ...
        csvfile.write(f"{material_id},{formula_pretty},{band_gap},{energy_above_hull},{density}\n")

# Print the summary table
print("{:<15} {:<15} {:<10} {:<20} {:<10}".format("Material ID", "Formula", "Band Gap", "Energy Above Hull", "Density"))
for material in mp_results:
    material_id = material.get("material_id", "")
    formula_pretty = material.get("formula_pretty", "")
    band_gap = material.get("band_gap", "")
...
# Retrieve plot selections from pipeline state
plot_selected = [
    {
      "plot_id": "plot_1",
    ...
    "plots_v1": [
        {
            ...
            "path": os.path.join("plots", f"{plot_spec["plot_id"]}_{title.lower().replace(' ', '_').replace(',', '')}.png"),
            "description": plot_spec["description"]
        }
        for plot_spec in plot_selected
    ]
