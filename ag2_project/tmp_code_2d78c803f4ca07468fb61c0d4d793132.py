...
for plot_spec in plot_selected:
    ...
    if is_v2:
        if plot_id not in plots_to_regenerate:
            continue
        out_folder = "plots_v2"
        out_path = os.path.join(out_folder, f"{plot_id}_{slug}.png")
    else:
        out_folder = "plots"
        out_path = os.path.join(out_folder, f"{plot_id}_{slug}.png")
    ...
    if plot_type == 'scatter_2d':
        x_key = axes['x']
        y_key = axes['y']
        x_data = [m.get(x_key, None) for m in mp_results if m.get(x_key, None) is not None]
        y_data = [m.get(y_key, None) for m in mp_results if m.get(y_key, None) is not None]
    ...
    # Histogram
        ...
        x_key = axes['x']
        hist_data = [m.get(x_key, None) for m in mp_results if m.get(x_key, None) is not None]
        if hist_data:
            plt.figure()
