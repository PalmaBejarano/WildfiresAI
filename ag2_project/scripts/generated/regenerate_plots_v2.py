...
# Check if we need to regenerate certain plots
is_v2 = isinstance(plots_to_regenerate, list) and len(plots_to_regenerate) > 0

def save_manifest(manifest, is_v2):
...
# Print fixed-width summary table
print(summary_df.to_string(index=False))

# Regenerate plots if required
manifest = []
for plot_spec in plot_selected:
    ...
    out_path = os.path.join(plots_v2_path if is_v2 else plots_v1_path, f"{plot_id}_{slug}.png")
    ...
    if plot_type == "scatter_2d":
        ...
        if x_data and y_data:
            ...
            plt.xlabel(axes["x"].capitalize().replace("_", " ") + " (eV)")
            plt.ylabel(axes["y"].capitalize().replace("_", " ") + " (g/cm³)")
            plt.title(title)
            plt.grid(True)
