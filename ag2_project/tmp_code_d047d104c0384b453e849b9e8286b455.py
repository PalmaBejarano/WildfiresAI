...
for plot_spec in plot_selected:
    ...
    if plot_type == "scatter_2d":
        ...
        if x_data and y_data:
            ...
            plt.xlabel(x_key.replace("_", " ") + " (eV/atom)" if "energy_above_hull" in x_key else " (eV)")
            plt.ylabel(y_key.replace("_", " ") + " (eV)" if "band_gap" in y_key else " (g/cm³)")
            plt.title(title)
            plt.grid(True)
        ...
        if hist_data:
            plt.figure()
            plt.hist(hist_data, bins=30, alpha=0.75)
            plt.xlabel(hist_key.replace("_", " ") + " (eV)" if "band_gap" in hist_key else " (eV/atom)" if "energy_above_hull" in hist_key else " (g/cm³)")
            plt.ylabel("Count")
            plt.title(title)
            plt.grid(True)
            plt.savefig(out_path)
