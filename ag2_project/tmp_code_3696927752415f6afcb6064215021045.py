...
for mat in filtered_materials:
    summary_rows.append([
        mat.get('material_id', ''),
        mat.get('formula_pretty', ''),
        mat.get('band_gap', ''),
        mat.get('energy_above_hull', mat.get('e_above_hull', '')),
        mat.get('density', '')
    ])
...
# Print to stdout
col_widths = [12, 25, 10, 20, 10]
headers = ['material_id', 'formula_pretty', 'band_gap', 'energy_above_hull', 'density']
header_line = "".join(f'{h:<{w}}' for h, w in zip(headers, col_widths))
print(header_line)
for row in summary_rows:
    print("".join(f'{str(item):<{w}}' for item, w in zip(row, col_widths)))

# Generate plots based on selected plot configurations

# Variable plot_selected is available from the runtime
...
with open('plots_v1_manifest.json', 'w') as manif_file:
    json.dump(manifest, manif_file, indent=4)
