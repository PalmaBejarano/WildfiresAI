...
# ---- Requirement: FIRMS_MAP_KEY must be provided via environment ----
# Option A (recommended): keep FIRMS_MAP_KEY in your local .env (NOT committed) and export it before running.
# Option B: export it in your shell profile, e.g. ~/.zshrc (also not committed).
: "${FIRMS_MAP_KEY:?FIRMS_MAP_KEY is not set. Export it first (or load it from your local .env).}"

# Optional: ensure conda is available; you can remove these lines if you always run inside the right env.
...
# Regions (broad BBOX per continent)
